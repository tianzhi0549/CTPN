...
class AccuracyLayer : public Layer<Dtype> {
 ...
 protected:
  ...
  bool is_ignore_label(const int label){
    const AccuracyParameter& accuracy_param=this->layer_param_.accuracy_param();
    for (int i=0; i<accuracy_param.ignore_label_size(); i++) {
      if (label==accuracy_param.ignore_label(i)) {
        return true;
      }
    }
    return false;
